...
String caption = "State, Tribe and Territory Budgets";
String pageTitle = "State, Tribe and Territory Budgets";
int count= 0;
...
StringBuffer pcScript = null;
     ...
     if (it.hasNext())
     {
      ...
      do
      {
       ...
       if (rs.getPlanStatus() == 1)
       {
			...
			stateList.append("<br />" + rs.getName() + "&nbsp; <a href=../"+ rs.getPlanUrl().trim()+" class='a1'  title='"+rs.getName().trim());
			if (rs.getPlanPeriod() != null) { 
				 stateList.append(" (Plan period: "+ rs.getPlanPeriod()+")' target='_blank'>Plan</a>");
			} else {
				 stateList.append("' target='_blank'>Plan</a>");
			}
 
			// HF - if the state has a budget, add the link here.  TODO: If this change will be permanent, add the link to the database and bean.
			if (rs.getState().equals("DE") )
				stateList.append(" | <a href='./budget_files/dcc_year2_rpt.pdf' class='a1' target='_blank' title='"+rs.getName().trim() + "'>Budget</a>");
			else if (rs.getState().equals("NH"))
				stateList.append(" | <a href='./budget_files/NH.zip' class='a1' target='_blank' title='"+rs.getName().trim() + "'>Budget</a>");
			else if (rs.getState().equals("IA"))
				stateList.append(" | <a href='./budget_files/Iowa Comprehensive Cancer Control Budget.xls' class='a1' target='_blank' title='"+rs.getName().trim() + "'>Budget</a>");
			else if (rs.getState().equals("WY"))
				stateList.append(" | <a href='./budget_files/Wyoming Comprehensive Cancer Control Budget.pdf' class='a1' target='_blank' title='"+rs.getName().trim() + "'>Budget</a>");
     ...
     myChart.appearanceFile = "apfiles/planet/ccpmap_budget.pcxml";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td><p><a href="../contact.html">Contact Us</a><br>
